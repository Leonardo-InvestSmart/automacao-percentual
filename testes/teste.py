...
def main():
    ...
    # Título com destaque igual à saudação
    st.markdown(
        """
        <h4 style="
    ...
    # Título fixo no topo da sidebar
    st.sidebar.markdown(
        """
        <style>
    ...
    # Escolhe ícone e renderiza o título
    icon = page_icons.get(pagina, "")
    st.markdown(
        f"<h1 style='color: black;'>{icon} {pagina}</h1>",
        unsafe_allow_html=True
    )
